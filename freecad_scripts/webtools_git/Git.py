...
class GitTaskPanel:
    ...
    def __init__(self,repo):
        ...
        QtCore.QObject.connect(self.form.buttonCheckout, QtCore.SIGNAL("clicked()"), self.checkout)
    ...
    def reopen(self):
        open_document_names = FreeCAD.listDocuments()
        open_document_paths = [FreeCAD.getDocument(doc_name).FileName for doc_name in open_document_names]
        for doc_name in open_document_names:
            doc = FreeCAD.getDocument(doc_name)
            FreeCAD.closeDocument(doc.Name)
        [FreeCAD.open(doc_path) for doc_path in open_document_paths]
        self.form.labelStatus.setText(translate("WebTools","Branch")+": "+self.repo.active_branch.name)
    ...
    def getBranches(self):
        self.form.listBranches.clear()
        if self.repo.remotes:
            for b in self.repo.branches:
                self.form.listBranches.addItem(b.name)
        else:
            FreeCAD.Console.PrintWarning(translate("WebTools","Warning: no remote repositories or branches")+".\n")
    ...
    def checkout(self):
        if len(self.form.listBranches.selectedItems()) != 1:
            FreeCAD.Console.PrintError(translate("WebTools","Please select a repo to pull from.")+"\n")
            self.form.labelStatus.setText(translate("WebTools","No repo selected"))
            return

        self.form.labelStatus.setText(translate("WebTools","Switching Branches"))
        b = self.form.listBranches.selectedItems()[0].text()
        s = self.repo.git.checkout(b)
        FreeCAD.Console.PrintMessage(translate("WebTools","Successfully switched to")+" "+b+"\n")
        self.form.labelStatus.setText(translate("WebTools",f'Branch: {b}'))
        self.reopen()
